...
{
    public sealed class ProjectilesHandler : IHandler
    {
        public Task ExecuteAsync(params string[] args)
        {
            var nodes = AssetFormatter.Assets["objects"].GetXmlsByNode("Class", "Projectile");
            ...
            AssetFormatter.ExportAssets("Projectiles", "Objects", assets, ref amount);

            Log.Info($"\t{amount} projectile{(amount > 1 ? "s" : "")}");
