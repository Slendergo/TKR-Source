...
{
    internal class GuildInviteHandler : PacketHandlerBase<GuildInvite>
    {
        ...
        private void Handle(Client src, string playerName)
        {
            ...
            foreach (var entry in src.CoreServerManager.ConnectionManager.Clients)
            {
                var client = entry.Key;
                if (client.Player == null || client.Account == null || !client.Account.Name.Equals(playerName))
                    continue;
